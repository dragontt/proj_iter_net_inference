...
../scripts/adjmtr2interactions.rb -a ${NETWORK} -r ${REGS} -c ${GENES} >> ${ADJLST}

## evaluate network in NetProphet style
Rscript ../scripts/evaluate_network.r ${ADJLST} ${CHIP_NET} ${PWM_NET} ${DIR_ANALYSIS}/evaluation.${NUMBINS}bins.top${MINRANK}to${MAXRANK}k.${NAME_ANALYSIS}.txt ${MAXRANK} ${NUMBINS}
